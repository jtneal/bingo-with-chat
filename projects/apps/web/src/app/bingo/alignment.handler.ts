...
export class AlignmentHandler {
  ...
  private static checkForOverflow(div: HTMLDivElement, fontSize: number): boolean {
    div.style.fontSize = `${fontSize}px`;

    return div.scrollWidth > div.clientWidth || div.scrollHeight > div.clientHeight;
  }

  private static calculateFontSize(div: HTMLDivElement, maxFontSize: number): number {
    ...
    while (high > low + 1) {
      ...
      [low, high] = AlignmentHandler.checkForOverflow(div, fontSize) ? [low, fontSize] : [fontSize, high];
    }

    return high;
  }

  private static setupFontStyles(div: HTMLDivElement, maxFontSize: number, minFontSize: number): number {
    const fontSize = Math.max(
      minFontSize,
      Math.min(AlignmentHandler.calculateFontSize(div, maxFontSize + 1) - 1, maxFontSize)
    );
    div.style.fontSize = `${fontSize}px`;
